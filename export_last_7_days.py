#!/usr/bin/env python3
"""
Export last 7 days timesheet (excluding today)
"""
...
def export_last_7_days_timesheet():
    """Export last 7 days timesheet (excluding today)"""
    
    print("正在导出过去7天 timesheet (不含当天)...")
    print("=" * 50)
    ...
    try:
        ...
        start_date, end_date = extractor._get_last_n_days_range(7)
        ...
        # Extract timeline data for last 7 days
        print("2. 开始提取过去7天的时间线数据...")
        ...
        for i, entry in enumerate(timeline_data.entries[:5], 1):
            ...
            print(f"      工时: {entry.work_load_hours:.2f}h - {entry.description[:50] if len(entry.description) > 50 else entry.description}")
        print()
        
        # Export to CSV
        print("4. 正在导出到 CSV 文件...")
        csv_path = exporter.export_timeline_to_csv(timeline_data, "last_7_days_timesheet.csv")
